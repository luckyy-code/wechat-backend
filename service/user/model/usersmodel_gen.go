...
type (
	usersModel interface {
		...
		SelectAllUserIdList(ctx context.Context) ([]int64, error)
	}
...
func (m *defaultUsersModel) SelectAllUserIdList(ctx context.Context) ([]int64, error) {
...
func (m *defaultUsersModel) Insert(ctx context.Context, data *Users) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.Username, data.Password, data.Nickname, data.Age, data.Avatar, data.Introduce, data.Email, data.State, data.IsDelete, data.LastTime, data.LastAddress)
	return ret, err
}

func (m *defaultUsersModel) Update(ctx context.Context, newData *Users) error {
	query := fmt.Sprintf("update %s set %s where `id` = ?", m.table, usersRowsWithPlaceHolder)
	_, err := m.conn.ExecCtx(ctx, query, newData.Username, newData.Password, newData.Nickname, newData.Age, newData.Avatar, newData.Introduce, newData.Email, newData.State, newData.IsDelete, newData.LastTime, newData.LastAddress, newData.Id)
	return err
}
